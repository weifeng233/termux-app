...
public class TermuxApiHandler {
    ...
    private static final String LOG_TAG = "TermuxApiHandler";
    ...
    public void sendToTermuxApi(Intent intent) {
        ...
        try {
            ...
            FileDescriptor outFileDescriptor = outputSocket.getFileDescriptor();
            ...
            LocalSocket inputSocket = new LocalSocket();
            inputSocket.connect(new LocalSocketAddress(inputSocketAddress));
            FileDescriptor inFileDescriptor = inputSocket.getFileDescriptor();
        ...
        if (!mServiceConnected) {
            try (PrintWriter out = new PrintWriter(new FileOutputStream(parcelOut.getFileDescriptor()))) {
                if (tryBindIfNecessary()) {
                    out.println("Establishing connection to Termux:API - try again in a few seconds");
                } else {
                    out.println("Termux:API is not installed or does not support this API");
                }
            }
            Log.e(LOG_TAG, "Not bound to Termux:API yet, ignoring intent");
        } else {
            Message msg = Message.obtain(null, 1, 0, 0, intent);
            try {
                mMessenger.send(msg);
            } catch (RemoteException e) {
                Log.e(LOG_TAG, "Exception sending message to termux-api", e);
            }
        }
    ...
    private static void tryClose(@NonNull Closeable c) {
        try {
            ...
            Log.e(LOG_TAG, "Unable to close API file descriptor", e);
        }
    }
    ...
    private final ServiceConnection mConnection = new ServiceConnection() {
        public void onServiceConnected(ComponentName className, IBinder service) {
            Log.e(LOG_TAG, "Connected to Termux:API service");
        ...
        public void onServiceDisconnected(ComponentName className) {
            Log.e(LOG_TAG, "Disconnected from Termux:API service");
            // This is called when the connection with the service has been
    ...
    public void handleApiIntent(Context context, Intent intent, String apiMethod) {
        try {
            ...
            Log.e(LOG_TAG, "Exception in handleApiIntent()", e);
        }
    }

    public boolean tryBindIfNecessary() {
        if (mServiceBindingInitiated) {
            Log.e(LOG_TAG, "Already started bind, no need to bind again");
            return true;
        }
        Log.e(LOG_TAG, "Trying to bind to Termux:API...");
        try {
            ...
            Log.e(LOG_TAG, "Exception binding to Termux:API service", e);
        }
        if (!mServiceBindingInitiated) {
            Log.w(LOG_TAG, "Could not bind to Termux:API");
        }
